...
class Movie:
    ...
    def get_movie_douban_top(self, page=1, num=10):
        cache_key = f"movie_top_{page}_{num}"
        cached = get_readis(cache_key)
        if cached:
            return cached

        start = (int(page) - 1) * int(num)
        end = int(page) * int(num)

        top_ids = list(
            MovieRatingDB.objects.order_by("-rating")
            .values_list("movie_id", flat=True)[start:end]
        )

        # 一次查出所有详情（不存在的会自动被过滤掉）
        movie_qs = CollectMovieDB.objects.filter(movie_id__in=top_ids)
        movie_json = queryset_to_json(movie_qs)

        # 按 top_ids 的顺序重排（否则 in 查询顺序不保证）
        mp = {m["movie_id"]: m for m in movie_json if "movie_id" in m}
        result = [mp[mid] for mid in top_ids if mid in mp]

        set_readis(cache_key, result)
        return result
