...
if(videos.length!=0) {
    ...
    for (let i = 0; i < videos.length; i++) {
        videos_txt = videos_txt + '<a target="_blank" title="'+videos[i]["source"]["name"]+'" href="'+videos[i]["sample_link"]+'" ><img alt="'+videos[i]["source"]["name"]+'" src="/api/img_proxy?url='+videos[i]["source"]["pic"]+'" ></a>';
...
for (let i = 0; i < photos.length; i++) {
    photos_txt  = photos_txt + '<a target="_blank" href="'+ photos[i]["alt"] +'"><img src="/api/img_proxy?url=' + photos[i]["thumb"] + '" noreferer="" /></a> ';
...
var movie_detail = '<div class="dyxingq"><div class="mi_ne_kd dypre">' +
    '<div class="dyimg fl"><img src="/api/img_proxy?url='+images["small"]+'"><div style="bottom: 2px; right: -8px; position: absolute;"><p class="movie_like '+movie_like+'" movieId="'+movie_id+'">&#10084;</p></div>';
