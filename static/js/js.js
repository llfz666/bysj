...
// 字符串转Json格式 - 支持Python字典格式(单引号)和标准JSON格式(双引号)
function strToJson(str1){
    ...
    try {
        ...
        // 如果失败，尝试处理Python格式的字典字符串（单引号）
        try {
            // 将单引号替换为双引号，但需要处理内部转义
            let str2 = str1
                .replace(/True/g, 'true')
            ...
            // 最后的备选方案：使用eval（兼容旧数据）
            try {
                let json = eval('(' + str1.replace(/True/g, 'true').replace(/False/g, 'false').replace(/None/g, "''") + ')');
                return json;
            } catch(e3) {
                console.warn('strToJson解析失败:', str1);
                return {};
            }
        }
    }
...
function movie_show(show_movie_data, show_movie_id) {
    ...
    let movie_search_show_text = '<ul class="movie-card-list">';
    let movie_id = "";
    let image = "";
    let title = "";
    let year = "";
    ...
    for (let j = 0; j < show_movie_data.length; j++) {
        ...
        let movie_like = "";
        if(typeof user_like !== 'undefined' && user_like.includes(movie_id)){
        ...
        // 仿豆瓣卡片结构
        movie_search_show_text = movie_search_show_text + '<li class="movie-card-item">';
        movie_search_show_text = movie_search_show_text + '<div class="movie-card">';
        movie_search_show_text = movie_search_show_text + '<a href="movie.html?id=' + movie_id + '" target="_blank" class="movie-card-link">';
        movie_search_show_text = movie_search_show_text + '<div class="movie-card-poster">';
        movie_search_show_text = movie_search_show_text + '<img class="movie-card-img" src="' + image + '" alt="' + title + '" loading="lazy">';
        if(rating) {
            movie_search_show_text = movie_search_show_text + '<span class="movie-card-rating">' + rating + '</span>';
        }
        if(year) {
            movie_search_show_text = movie_search_show_text + '<span class="movie-card-year">' + year + '</span>';
        }
        movie_search_show_text = movie_search_show_text + '</div>'; // end poster
        movie_search_show_text = movie_search_show_text + '<div class="movie-card-info">';
        movie_search_show_text = movie_search_show_text + '<h3 class="movie-card-title" title="' + title + '">' + title + '</h3>';
        movie_search_show_text = movie_search_show_text + '<p class="movie-card-actors" title="' + actors + '">' + actors + '</p>';
        movie_search_show_text = movie_search_show_text + '</div>'; // end info
        movie_search_show_text = movie_search_show_text + '</a>';
        movie_search_show_text = movie_search_show_text + '<p class="movie_like '+movie_like+'" movieId="' + movie_id + '">&#10084;</p>';
        movie_search_show_text = movie_search_show_text + '</div>'; // end card
